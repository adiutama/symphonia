...
**Switch Worktree** / **Switch Workspace** also only hide — the Editor PTY stays alive until the Operator **Close Overlay**s it, the owning Worktree / Workspace is removed, or the app quits. Overlay processes are as durable as Main CLI for that session.
