...
| `Command` | Stable string `id`, title, optional subtitle/group, `defaultAliases: [String]`, `defaultShortcut: String?`, `isEnabled(CommandContext) -> Bool`, and a `run` that wraps an existing `CommandModeAction` |
...
| `CommandBindingOverride` | Codable Global Setting override for one Command's aliases/shortcut, keyed by Command `id` (ADR 0021 CC.3) |
| `CommandBindingResolver` | `aliases(for:overrides:)` / `shortcut(for:overrides:)` — effective binding = override ?? Command default |
...
`CommandRegistry` is constructed once in `SymphoniaApp.init()` (providers: Workspace, Overlay, Chrome) and injected into `CommandModeController`, which drives its root palette entirely from `commandRegistry.availableCommands(context:)` — the old private `rootItems()` / `slashCommands` table is gone. Filtering matches title, subtitle, **and every effective alias** (free text, leading `/` optional) via simple case-insensitive substring containment. An empty filter shows every enabled Command; sequences and recorded chords fire from the key monitor per ADR 0022. Subtitle is overridden with live `CommandContext` state only where providers need it (e.g. effective editor command) — everything else uses the provider's own subtitle text.

**Operator overrides (ADR 0021 CC.3):** `GlobalPreferences.commandBindings: [String: CommandBindingOverride]` persists per-Command alias/shortcut overrides at `~/.symphonia/preferences.toml` under `[commandBindings."<id>"]` tables, keyed by stable Command `id`. `aliases` is stored as a single comma-separated string (`nil` = Command defaults; `""` = no aliases; parsed via `CommandBindingResolver.parseAliases`, splitting on `,` and trimming). `shortcut` is `nil` = Command default, `""` = explicitly no shortcut, otherwise the override key. `CommandModeController` reads `preferences.preferences.commandBindings` on every rebuild (including a Combine sink on `preferences.$preferences`) and resolves effective aliases/shortcuts via `CommandBindingResolver` instead of `Command.defaultAliases` / `defaultShortcut` directly. Legacy `preferences.json` is ignored.

**Settings UI (ADR 0021 CC.4):** `CommandBindingsSettingsView` (`App/`) is a new Global Settings nav item ("Commands", between Base Ref and Effective Setting) listing every `CommandRegistry.allCommands`, grouped by `Command.group` (ungrouped → "Other"), with a live search filter over title/id. Each row shows title + id (caption) and two live-bound fields — Aliases (comma-separated free text, placeholder = defaults joined by `, `) and Shortcut (placeholder = default shortcut or "none") — that write straight into `preferences.preferences.commandBindings[id]`, matching the existing Global Settings "bind live + Save" pattern (the pane sits inside the same `globalForm`, so it gets the existing Save / Reload / Reset Global chrome for free). Typing an empty value is saved as an explicit override (`""`, i.e. "no aliases" / "no shortcut" per CC.3 semantics) rather than clearing it; each row with an active override shows a **Reset** button that removes its `commandBindings` entry entirely to fall back to the Command's own defaults — this is the only supported way back to "use defaults" once an override exists. A non-blocking **Conflicts** section lists Commands that share the same effective alias (case-insensitive, trimmed) or the same non-empty effective shortcut, computed over the whole registry (not just the filtered/search view); conflicts are surfaced but do not block editing or Save.
